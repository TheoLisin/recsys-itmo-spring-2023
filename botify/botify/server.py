...
from botify.recommenders.contextual_history import MyRecommender
...
class NextTrack(Resource):
    def post(self, user: int):
        ...
        treatment = Experiments.HISTCONTEXT.assign(user)
        if treatment == Treatment.T1:
            recommender = MyRecommender(tracks_redis.connection, tracks_with_diverse_recs_redis.connection, catalog)
        else:
